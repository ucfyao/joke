...
class Image {
    ...
    static public function water($source, $water, $savename=null, $alpha=80) {
		...
		if ($sInfo['type'] == 'jpg' || $sInfo['type'] == 'jpeg') {
			imagejpeg($sImage, $savename, 100);//第3个参数即使质量大小，因为只有imagejpeg支持这个参数
		}else{
    ...
    static function thumb($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        ...
        if ($info !== false) {
			...
			//防止用户上传图片过小，背景会变黑。设置背景颜色
			$bg =imagecolorallocate($thumbImg, 255, 255,255);
			imagefill($thumbImg, 0, 0, $bg);
			
           // 复制图片
			if (function_exists("ImageCopyResampled")){
				imagecopyresampled($thumbImg, $srcImg,0,0,$array['left'],$array['top'],$width,$height,$array['width'],$array['height']);
			}else{
				imagecopyresized($thumbImg, $srcImg,0,0,$array['left'],$array['top'],$width,$height,$array['width'],$array['height']);
			}
