...
class class_video{
	...
	**/
	function youku( $vid ) {	
		...
		$r['url'] = 'http://v.youku.com/v_show/id_'. $json['vidEncoded'] .'.html?ref=http://www.loowei.com/';
		$r['swf'] = 'http://player.youku.com/player.php/sid/'. $json['vidEncoded'] .'/loowei.swf';
		$r['title'] = $json['title'];
	...
	**/
	function tudou( $vid ) {
		...
		$r['vid'] = $arr['code'];
		$r['url'] = 'http://www.tudou.com/programs/view/'. $arr['code'] .'/?FR=http://www.loowei.com/';
		$r['swf'] = 'http://www.tudou.com/v/'. $arr['code'] .'/loowei.swf';
		$r['title'] = $arr['title'];
	...
	**/
	function ku6( $vid ) {
		...
		$r['vid'] = $vid;
		$r['url'] = 'http://v.ku6.com/show/'. $vid .'.html?ref=http://www.loowei.com/';
		$r['swf'] = 'http://player.ku6.com/refer/'. $vid .'./loowei.swf';
		$r['title'] = $json['t'];
	...
	**/
	function _56( $vid ) {
		...
		$json = $json['info'];
		$r['vid'] = $json['textid'];
		$r['url'] = 'http://www.56.com/u/v_'. $json['textid'] .'.html?ref=loowei.com';
		$r['swf'] = 'http://player.56.com/v_'. $json['textid'] .'.swf?ref=loowei';
	...
	function sina( $vid ) {
		...
		if ( !preg_match( '/^[0-9]+$/i', $vid ) ) {
			...
			if ( preg_match( '/^http\:\/\/video\.sina\.com\.cn\/v\/b\/(\d+)-(\d+)/i', $vid, $match ) || preg_match( '/^http\:\/\/you\.video\.sina\.com\.cn\/api\/sinawebApi\/outplayrefer\.php\/vid=(\d+)_(\d+)_([0-9a-zA-Z+%]+)/i', $vid, $match ) ) {
				...
				if ( $uid != 1 ) {
					$url = 'http://video.sina.com.cn/v/b/'. $vid .'-'. $uid .'.html?ref=loowei.com';
				}
			} else {
				return false;
			}
	...
	**/
	function qq( $vid ) {
		...
		$r['vid'] = $xml['vid'];
		$r['url'] = 'http://v.qq.com/page/t/u/h/'. $xml['vid'] .'.html?ref=loowei.com';
		$r['swf'] = 'http://static.video.qq.com/TPout.swf?vid='. $xml['vid'] .'&ref=loowei.com';
		$r['title'] = $xml['ti'];
	...
	**/
	function sohu( $vid ) {
		...
		$r['vid'] = $vid;
		$r['url'] = $json['url'] . '?ref=loowei.com';
		$r['swf'] = 'http://share.vrs.sohu.com/my/v.swf&ref=loowei.com&id=' . $vid;
