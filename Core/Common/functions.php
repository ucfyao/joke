...
function get_list_url($a,$m = ''){
	if($GLOBALS['config']['site_html']){
		if(empty($m)){
			return $GLOBALS['config']['site_url'].'/'.$a.'/';
		}else{
			return $GLOBALS['config']['site_url'].'/'.strtolower($m).'/';
		}
	}else{
		if(empty($m)){
			return U('Index/'.$a);
...
function get_user_center(){
	if($GLOBALS['config']['site_html']){
		return $config['site_url'].'/users/'.$_SESSION['user']['uid'].'/';
	}else{
		return U('Users/index?uid='.$_SESSION['user']['uid']);
	}
}
function get_user_avatar($size){
	if($GLOBALS['config']['site_html']){
		return $config['site_url'].'/avatar/'.$size.'/'.$_SESSION['user']['uid'].'.jpg';
	}else{
		return U('Avatar/index?size='.$size.'&uid='.$_SESSION['user']['uid']);
	}
}