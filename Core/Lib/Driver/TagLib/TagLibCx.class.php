...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
		...
		//自定义标签
		'article_list' => array('attr'=>'type,day,hour,order,limit,page,rows,user,condition,url','level'=>3), //列表 - hour优先于day
		'hot_tag_list' => array('attr'=>'limit','level'=>3), //热门标签列表
    );
	/**
     * 获取标签列表
     * @access public
     ...
     * @return string|void
     */
	public function _hot_tag_list($attr,$content){
		$tag   =    $this->parseXmlAttr($attr,'hot_tag_list');
		if(!isset($tag['limit'])){$tag['limit'] = 0;}
		
		$sql = "D('Hot_tag')->get_lists(".$tag['limit'].")";
		
		$parsestr = '<?php $_result='.$sql.';if(!is_array($_result["hot_tag_list"])) : echo "" ; else: $i=0;$more_tag_url=$_result["more_tag_url"];';
		$parsestr .= 'foreach($_result["hot_tag_list"] as $key=>$value):';
		$parsestr .= '++$i;?>';
		//解析在标签中的内容
		$parsestr .= $content;
     ...
     * @return string|void
     */
	public function _article_list($attr,$content){
		$tag   =    $this->parseXmlAttr($attr,'article_list');
		if(!isset($tag['type'])){$tag['type'] = -1;}elseif($tag['type'] < 0 || $tag['type'] > 2){echo '内容列表的类别只允许0-2之间，或者不填写！';exit;}
		if(!isset($tag['day'])){$tag['day'] = 0;}
		if(!isset($tag['hour'])){$tag['hour'] = 0;}
		if(!isset($tag['order'])){$tag['order'] = "`time` DESC";}
		if(!isset($tag['limit'])){$tag['limit'] = 0;}
		if(!isset($tag['page'])){$tag['page'] = 0;}
		if(!isset($tag['rows'])){$tag['rows'] = 0;}
		if(!isset($tag['user'])){$tag['user'] = 0;}
		if(!isset($tag['condition'])){$tag['condition'] = 0;}
		if(!isset($tag['url'])){$tag['url'] = ACTION_NAME;}
		if(empty($tag['limit']) && empty($tag['page'])){echo '如果不对内容进行翻页，请设置取出多少条内容！';exit;}
		elseif(!empty($tag['limit']) && !empty($tag['page'])){echo '取出内容条数和翻页只能二选一，请用 rows 设置列表翻页行数！';exit;}
		
		$sql = "D('Article')->get_lists(".$tag['type'].",".$tag['day'].",".$tag['hour'].",'".$tag['order']."',".$tag['limit'].",".$tag['page'].",".$tag['rows'].",".$tag['user'].",'".$tag['condition']."','".$tag['url']."')";
		
		$parsestr = '<?php $_result='.$sql.'; if(!is_array($_result["article_list"])) : echo "<div class=list_no_content>没有内容了，请看看其他的！</div>" ; else: $i=0;$article_page=$_result["page"];';
		$parsestr .= 'foreach($_result["article_list"] as $key=>$value):';
		$parsestr .= '++$i;?>';
		//解析在标签中的内容
		$parsestr .= $content;
		$parsestr .= '<?php endforeach; endif;?>';
